...
const removeDuplicateLetters = function(s) {
	...
	//注意这时 i=-1, counts对象表示还i之后对应的字母的个数
	while (i++<s.length){
...
1. 我们这里没办法像前面的单调栈一样只循环一次就出结果，这里有两个数组，是无法一下子决定哪个数组要多少个数字能得到最优解。因此我们需要设置所有的情况来进行讨论！！！
2. 对两个数组分别计算出各自的序列后在进行合并
...
* 放入p1的条件是 p2 大于等于 len2, 这个需要放在前面。因为while保证了p1和p2至少有一个没越界，而这里的 if p2>=len2 则说明 p1必定没越界, 那么只能加入 p1。其次 nums[p1] > nums[p2] 没办法判断哪个越界，当越界的时候 值为 undefined 而在进行比较运算时会转化 Number(undefined) 为NaN，也就是说如果有越界的话 nums1[p1] > nums2[p2] 由于存在一个NaN必定为false，但我们却没法判断哪个越界，因此必须把 p2>=len2的判断放在前面。
...
🌟这里很巧妙的地方在于 <span style="color:blue ;"> while(\_p1 < len1 && nums1[++\_p1] === nums2[++_p2]) {}</span>  这里直接可以计算到最后不相等的时候或者都到尽头的时候

🌟push也是很巧妙同样通过越界和大小一起l来判断
